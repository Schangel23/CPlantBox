...
void Plant::initCallbacks()
{
    ...
    for (auto& p_otp :organParam[Organism::ot_stem]) {
        ...
        tropism->setGeometry(geometry);
        rp->f_tf = tropism; // set new one
        // S0.5 dispatch (ADR_LEAF_KINEMATICS_2026-04-28). When the LRP has
        // the FA kinetics flag set, mint MultiPhaseStemGrowth in place of
        // the XML `gf` field. Non-FA XMLs (wheat, brassica, modelparam_4,
        // …) keep their native ExponentialGrowth/Linear/etc. dispatch —
        // MultiPhaseStemGrowth returns `p.lb + min(fa_sum, Σ p.ln)` which
        // only reproduces the legacy length curve when the FA pathway is
        // actually populating `fa_sum`. Bit-identical regression on native
        // paths (CLAUDE.md principle 6) requires this gate.
        const bool fa_on = rp->use_fournier_andrieu_kinetics;
        const int gft_eff = fa_on ? gft_multi_phase_stem : rp->gf;
...
std::shared_ptr<Tropism> Plant::createTropismFunction(int tt, double N, double sigma, double ageSwitch) {
	...
	case tt_prescribed_midrib: return std::make_shared<PrescribedMidribTropism>(shared_from_this(),N,sigma); // steers growth along the leaf's surface_cps midrib (exact, deterministic)
